	...
	*( (int *)pptr->pbase ) = MAGIC;
    ...
    while (page_table_index < 4)
    {
        ...
        while (entry_index < 1024)
        {
            ...
            pte->pt_global = 1;  
            pte->pt_avail = 0; 
            pte->pt_base = (page_table_index * FRAME0) + entry_index;  
            pte++;
            entry_index++;
        }    
	...
	get_frm(&fr);	
	proctab[NULLPROC].pdbr = (fr + FRAME0) * NBPG;
	...
	while(fr_index < 1024)
	{
		pde[fr_index].pd_write = 1;
		
		if (fr_index < 4)
		{
			pde[fr_index].pd_pres = 1;
			pde[fr_index].pd_base = FRAME0 + fr_index;	
		} 	
